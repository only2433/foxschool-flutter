...
class QuizTextPlaySubScreen extends ConsumerStatefulWidget {
  ...
  @override
  QuizTextPlaySubScreenState createState() => QuizTextPlaySubScreenState();
}

class QuizTextPlaySubScreenState extends ConsumerState<QuizTextPlaySubScreen> with TickerProviderStateMixin{
  ...
  Widget build(BuildContext context) {
    final interactionState = ref.watch(quizUINotifierProvider.select((it) => it.userInteractionState));
    return Container(
              ...
              Column(
                children: [
                  ...
                  SizedBox(
                      ...
                      child: Column(
                        children: _buildTextExampleItemView(context, interactionState),
                      )
                  ),
                  _buildNextButtonView(context, interactionState)
                ],
              ),
              _buildCheckAnimationView(context, interactionState)
            ],
          )
        ],
      ),
    ...
    for(int i = 0 ; i < widget.data.exampleList.length ; i++)
    {
      ...
      result.add(QuizExampleTextWidget(
          ...
          onItemPressed: () {
            ...
            widget.quizController.onSelectTextQuizData(index, widget.data, widget.data.exampleList[index].isAnswer);
          },
